...
tags:
  ...
  - 勉強会資料
private: false
updated_at: '2026-05-15T09:28:04+09:00'
id: 3f2a397e944401fcc6cb
...
`if`等の条件分岐を使わずに引数の構造で関数定義を場合分けできることがわかる。
...
例えば、AIに書かせた便利ツールやGitHubで公開されているツールを会社で使う際に、「これはローカルでしか動作しないはずだが機微情報を扱う関係上、外部と通信していないか念の為調べてから導入するように」と言われた際にはモナドがある=副作用がある場所を重点的にチェックすればよい。
これは、会社人的には結構嬉しいのではないだろうか。

:::note info
関数型言語ではないが、[Deno](https://deno.com/)というJSのランタイムにも実行時に副作用を明示的に指定する機能があるので、実行時にxxしないことを保証したいならこちらを使う方が楽だと思われる。
:::
...
- 業務アプリを作る際には、冪等である場所が壊されないように隔離するように意識しており、純粋関数を使うHaskellの考え方に通づるものがあるのではないか。
...
## Haskellに興味を持った方がいれば読むと良いもの

自分が見ると良いと思った順で書いておく。

- [私がHaskell学習を推す理由](https://www.youtube.com/watch?v=z_CraKpnfuc): 元MetaエンジニアがHaskellを推す理由を語ってくれるYouTube動画
...
- [すごいHaskell入門](https://tatsu-zine.com/books/sugoi-haskell-ja): ※有料。ポップなイラストに反してめちゃくちゃわかりやすく、深い所まで知れる良い入門書。Haskellの入門書という意味では一番おすすめ。
