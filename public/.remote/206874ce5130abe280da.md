...
tags:
  ...
  - 正格評価
private: false
updated_at: '2026-08-02T14:14:41+09:00'
id: 206874ce5130abe280da
...
https://qiita.com/sigma_devsecops/items/8dfa8836e4ae44c3e3c1
...
遅延評価により、`acc`は再帰呼出しが行われるたび即時評価されない。
そのため、`((((0+1)+2)+3)+4)`の巨大サンクがヒープに貯まる。
...
`foldr`は`foldl`と比較して巨大サンクが作成されない。
一方でスタックフレームとして結果待ちのリソースがスタックに積まれるという問題がある。

TODO: このあたりの問題について後日記事を書く予定。

### `foldr`は計算を途中で打ち切ることができる場合がある
...
2026年7月追記: 最近`foldr`の短絡についての記事を書いたのでlinkを貼っておく。

https://qiita.com/sigma_devsecops/items/3234a53bb3d0e2d8ab8f

### `foldr`は演算子がstrictな場合、末尾まで再帰展開が必要なため、遅い
...
この連鎖が深くなると、評価時に再帰の戻り(stack unwind)が必要になり、実装によってはスタックオーバーフローを引き起こす。
...
### 2026年6月追記: 途中で畳み込みを打ち切るには`foldr`が使える

https://atcoder.jp/contests/abc109/tasks/abc109_b

途中で条件を満たさないことがわかった場合に、最後まで畳み込みを実施せずに途中で打ち切りたい場合がある。
このような際には、`foldr`の活躍のチャンスである。
...
```
`foldl`や`foldl'`の場合には左畳み込みなので、畳み込みを途中で止めることはできない。
...
Haskellには[`all`](https://hackage-content.haskell.org/package/base-4.22.0.0/docs/Data-List.html#v:all)という全てがTrueのリストか確認する関数があるので、これを使うほうが素直かもしれない。

簡略化して書くと、`all`は以下のようなことを行っている。
...
:::note info
これを見て、なんでこれで途中で処理が打ち切れるんだ?と思った人向けに解説を書いておく。

以下の例を愚直に展開してみる。

```
all (>2) [3,2,1] = foldr (\x e -> (>2) x && e) True [3,2,1]
```

まず、`foldr`を下記のように展開していく。

```haskell
foldr _ e [] = e
foldr k e (x : xs) = k x (foldr k e xs)
```

そのため、例の式はラムダ式`(\x e -> f x && e)`を`k`とすると以下のように展開できる。

```
k 3 (foldr k True [2,1])
```

ここで`k`が`(\x e -> (>2) x && e)`であることを思い出す。

```
(\x e -> (>2) x && e) 3 (foldr (\x e -> (>2) x && e) True [2,1])
```
ラムダ式を評価すると、

```
= (>2) 3 && (foldr (\x e -> (>2) x && e) True [2,1])
= True && (foldr (\x e -> (>2) x && e) True [2,1])
```
`foldr`をもう一つ展開すると以下になる。

```
= True && (False && (foldr (\x e -> (>2) x && e) True [1])))
```

`&&`は遅延評価により、左側が`False`の場合、右側を評価せず、`False`を返す。
そのため、リストの`2`をラムダ式(`k`)の結果が`False`になった段階でそれ移行の`foldr`は評価されない。
:::

`all`は内部で畳み込みを行っているが、引数に対して直接畳み込みを行うわけではないので、`foldr`の実装から少し書き換えて以下のように実装できる。
