...
tags:
  ...
  - hsts
private: false
updated_at: '2026-02-25T11:41:26+09:00'
...
## おまけ: HTTPSポートのみが開いており、HTTPポートがクローズでもHSTSすべき

https://cheatsheetseries.owasp.org/cheatsheets/HTTP_Headers_Cheat_Sheet.html#strict-transport-security-hsts

中間者攻撃(MITM)を想定すると、実施すべき。

- `Secure`属性なしのCookieがブラウザに保存されている場合
- ユーザは正規のサーバの80番ポートに接続しているつもりで中間者の80番ポートにアクセスしている。
- `HSTS`が設定されていれば、HTTP通信の発生を防ぐことができ、結果として`Secure`属性なしCookieの送信を防ぐことができる。
...
participant "Legitimate Server\n(HTTPS only)" as S

note right of A
  MITMがHTTPSに変換し、リダイレクトする
end note

U -> A : HTTP Request\n(GET /)\nCookie: session=abc123
A -> S : HTTPS Request\n(GET /)
S --> A : HTTPS Response\n(Set-Cookie / HTML)
A --> U : HTTP Response\n(Modified / Stripped Redirect)
