...
tags:
  ...
  - 純粋関数
private: false
updated_at: '2026-06-18T23:15:36+09:00'
...
つまり、`mapAccumL go 初期状態 xs`のように定義した場合には`go :: 状態 -> 入力 -> (更新された状態, 出力)`のような`go`関数を定義する必要がある。
逆に`(更新された状態, 出力)`第二要素である出力は追加していく(収集される)だけの動きになるので次の関数呼び出しから参照することができない。参照が必要な場合には`((更新された状態, 前の出力), 出力)のように第一要素側に追加する必要がある。

これを使用して書き換えた挿入ソートが以下。
...
胡椒のストックを状態を更新しつつ、出力として今回使用した胡椒を出力している。
