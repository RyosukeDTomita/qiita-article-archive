...
tags:
  ...
  - 計算量
private: false
updated_at: '2026-02-22T10:59:58+09:00'
...
この問題は末尾からマッチする文字列があるかを探す必要があるのだが、以下のように素直に末尾からマッチするものを探すと、1回の調査で$O(n)$になるため、全体では$O(n^2)$になってしまい、TLEになってしまう。
...
先に全体をreverseで反転しておくことで先頭から要素をマッチさせることができ、1回の試行が$O(1)$になるため、全体としては$O(n)$になり、TLEを回避できる。
...
Haskellのリスト及び、`Data.Vector`[^3]を使用しても末尾への連結はO(n)になる。

何度も末尾連結が発生する場合には、以下のどちらかを実施すると計算コストが軽くなる。

- 先頭に追加していき、最後に逆順にする(上記)
- [`Data.Sequence`](https://hackage-content.haskell.org/package/containers-0.8/docs/Data-Sequence.html)を使う
...
[^3]: https://hackage-content.haskell.org/package/vector-0.13.2.0/docs/Data-Vector.html#g:12
